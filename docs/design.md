...
2. **Codification/extraction version** — *git history.* Each accepted extraction is tagged `pe-us-<version>` (e.g. `pe-us-1.155.0`) after CI passes. The manifest records, per extraction: the pe-us version, the pe2lean version, the extraction date, content hashes of every source formula, and the **law-date coverage window** (the min/max effective dates present in the extracted parameters).
