...
| 16 | Lean 4.32.1's runtime breaks at ~3,200-definition generated-code scale: reproducible segfaults in the interpreter (`utf8_char_pos` null-deref), under precompiled dynlibs, and in fully compiled executables (mimalloc corruption, not OOM) — the identical pipeline is green at 1,771 defs. Scale-triggered toolchain bug; minimal repro + upstream report queued | T4 (reproduced) | Phase B2 (branch) |
